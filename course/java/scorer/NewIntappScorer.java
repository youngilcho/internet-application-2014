...
public class NewIntappScorer extends ScoringFunctionIterator {
    ...
    public NewIntappScorer(Parameters parameters, CountIterator iterator) throws IOException {
        super(iterator);
        ...
        if(termPositions != null && count != 0) {
            for(int i = 0; i < termPositions.size(); i++) {
                ...
                if (length < 30) {
                    ...
                    value = ((double)(length - termPositions.get(i)) / (double)length + 1.0) / 1.3; // NOTE 대강봤을 때, 200이상이 롱테일 뒤에 위치할수록 가중치 더 큼
                }
